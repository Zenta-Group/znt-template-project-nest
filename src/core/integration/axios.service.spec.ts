...
describe('AxiosService - Métodos HTTP y manejo de errores', () => {
  ...
  it('should handle error on post', async () => {
    axiosInstance.post.mockRejectedValue({ response: { status: 400, data: { message: 'bad' } }, message: 'bad', stack: 's' });
    await expect(service.post('/fail', {})).rejects.toThrow(BadRequestException);
  });
  ...
  it('should handle error on put', async () => {
    axiosInstance.put.mockRejectedValue({ response: { status: 401, data: {} }, message: 'unauth', stack: 's' });
    await expect(service.put('/fail', {})).rejects.toThrow(UnauthorizedAccessException);
  });
  ...
  it('should handle error on delete', async () => {
    axiosInstance.delete.mockRejectedValue({ response: { status: 404, data: {} }, message: 'not found', stack: 's' });
    await expect(service.delete('/fail')).rejects.toThrow(ResourceNotFoundException);
  });
  ...
  describe('setupAuthenticationInterceptor', () => {
    it('should set API key header', () => {
      const s = new AxiosService('url', { type: SecurityType.API_KEY, apiKey: 'abc' });
      expect(() => s.onModuleInit()).not.toThrow();
      // @ts-ignore
      expect(s["axiosInstance"].defaults.headers.common['x-api-key']).toBe('abc');
    });
    ...
    it('should set Bearer token header', () => {
      const s = new AxiosService('url', { type: SecurityType.BEARER_TOKEN, token: 'tok' });
      expect(() => s.onModuleInit()).not.toThrow();
      // @ts-ignore
      expect(s["axiosInstance"].defaults.headers.common['Authorization']).toBe('Bearer tok');
    });
    ...
    it('should set Cloud Run local ID token', () => {
      const s = new AxiosService('url', { type: SecurityType.GOOGLE_CLOUD_RUN_AUTH, cloudRunIdToken: 'idtok' });
      expect(() => s.onModuleInit()).not.toThrow();
      // @ts-ignore
      expect(s["axiosInstance"].defaults.headers.common['Authorization']).toBe('Bearer idtok');
    });
    it('should throw if Cloud Run target url missing', () => {
      const s = new AxiosService('url', { type: SecurityType.GOOGLE_CLOUD_RUN_AUTH });
      expect(() => s.onModuleInit()).toThrow('URL de destino de Cloud Run no proporcionada.');
    });
    it('should do nothing for NONE', () => {
      const s = new AxiosService('url', { type: SecurityType.NONE });
      expect(() => s.onModuleInit()).not.toThrow();
      // @ts-ignore
      expect(s["axiosInstance"].defaults.headers.common['Authorization']).toBeUndefined();
    });
